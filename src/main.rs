...
// Package so we don't need to put
// this at top of every module!!
macro_rules! errorln(
    ($($arg:tt)*) => ( {
      use std::io::Write;
...
fn echo_requests(server: tiny_http::Server) {
    ...
    if r.url() == "/favicon.ico" {
      r.respond( tinier::hdr_response(404, Vec::with_capacity(0)) );
      continue;
    }
    ...
    if r.method().eq(&*tinier::GET) || r.method().eq(&*tinier::PUT) {
      ...
      r.respond(tinier::str_response(200, headers, html_str));
    }
    println!("");
  }
}
